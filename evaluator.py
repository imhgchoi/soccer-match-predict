...
class Evaluator():
    ...
    def profitability(self):
        ...
        plt.title('Train Set Bet Backtest [{}]'.format(self.config.model_type))
        ...
        print('Test Copycat Net Profit : ', copy_profit)
        print('Test Copycat Return on Investment [ Net Profit / Total Money Invested ] : ', copy_profit / (self.config.bet_money * test_res.shape[0]))
        plt.plot(test_profit_list[int(len(test_profit_list)*0.1):])
        plt.plot(rand_profit_list[int(len(rand_profit_list)*0.1):])
        plt.plot(copy_profit_list[int(len(copy_profit_list)*0.1):])
        plt.title('Test Set Bet Backtest [{}]'.format(self.config.model_type))
        plt.xlabel('Soccer Match Sequence')
        plt.ylabel('Return on Investment [ Net Profit / Total Money Invested ]')
